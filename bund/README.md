...
- `vehicles.py` 制作带轮胎、五辐轮毂、车窗、灯组、门缝、后视镜和格栅的四门轿车；轮胎最低点为模型原点，场景放置在 0.02 米路面。
- `streets.py` 生成路侧 3 米宽人行道，高出柏油路 0.15 米；扣除相交车道和已生成的步道，保持路口通行。桥上保留独立标高。运行 `python assets/bund/streets.py` 检查路口裁剪。
- 命名地标与背景建筑底部落到 0 米地面；同步重建 `.blend`、组合 GLB 与清单。
- `export-runtime.py` 输出 198 个约 250 米城市分块，采用 20 位 Draco 位置量化，保留薄立面细节；人行道另按相同网格分块，共用按距离和视野加载的流程。`world.json` 包含分块包围球、地面实体碰撞数据、设施和水域坐标。
- 柏油路导出为实体薄层；物理数据使用闭合长方体，人行道使用凸多边形柱体，绿地使用三角柱。建筑碰撞仍为室外包围体。
